...
document.addEventListener('wsx:json', (event) => {
  const { id, channel, data, metadata } = event.detail;
  console.log(`JSON message ${id} on ${channel}`, data, metadata);
});
```

Pair DOM listeners with programmatic handlers registered via `wsx.onJson()` for
...
document.addEventListener('wsx:stream', (event) => {
  const { channel, metadata, arrayBuffer } = event.detail;
  console.log(`Stream chunk on ${channel}`, metadata);

  if (metadata?.mimeType?.startsWith('audio/')) {
    const audio = document.querySelector('#preview');
    audio.src = URL.createObjectURL(new Blob([arrayBuffer], { type: metadata.mimeType }));
  }
});
```

Use `wsx.onStream()` when you prefer callback registration and the `wsx:stream`
event when you need declarative DOM hooks.
